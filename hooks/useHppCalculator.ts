...
export function useHppCalculator() {
  const [calculation_method, set_calculation_method] = useState<
    "full" | "variable"
  >("full");
  ...
  const summary = useMemo(() => {
    ...
    const total_bop_fix =
      calculation_method === "full" ? calculate_total(bop_fix_items) : 0;

    const total_hpp =
      total_bbb + total_btkl + total_packaging + total_bop_var + total_bop_fix;
