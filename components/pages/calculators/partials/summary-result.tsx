"use client";

import { formatIDR } from "@/lib/format-currency";
import { Star } from "lucide-react";

interface SummaryResultProps {
  calculation_method: "full" | "variable";
  production_unit?: string;
  production_qty?: number | string;
  margin_percentage?: number;
  stepNumber?: string | number;
  summary: {
    total_bbb: number;
    total_btkl: number;
    total_packaging: number;
    total_bop_var: number;
    total_bop_fix: number;
    total_hpp: number;
    hpp_per_unit: number;
    profit_margin_value: number;
    recommended_price: number;
    total_revenue: number;
    total_profit: number;
  };
}

export default function SummaryResult({
  summary,
  calculation_method,
  production_unit = "Unit",
  production_qty = 0,
  margin_percentage = 0,
  stepNumber = 6,
}: SummaryResultProps) {
  const unitLabel = production_unit || "Unit";
  const qtyNumber = Number(production_qty) || 0;

  return (
    <div className="mb-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm xl:p-6">
      <div className="mb-6 flex items-center gap-3.5">
        <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-blue-600 text-[13px] font-bold text-white shadow-sm">
          {stepNumber}
        </div>
        <h2 className="text-[15px] font-bold text-slate-800">
          Hasil Kalkulasi HPP
        </h2>
      </div>
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2 xl:gap-8">
        <div className="flex flex-col">
          <h3 className="mb-4 text-[11px] font-bold uppercase tracking-wider text-slate-400">
            Rincian Biaya
          </h3>

          <div className="flex flex-col gap-3.5">
            <div className="flex items-center justify-between text-[13px]">
              <div className="flex items-center gap-2.5">
                <span className="h-2 w-2 rounded-full bg-blue-500"></span>
                <span className="text-slate-500">Biaya Bahan Baku (BBB)</span>
              </div>
              <span className="font-bold text-slate-800">
                {formatIDR(summary.total_bbb)}
              </span>
            </div>

            <div className="flex items-center justify-between text-[13px]">
              <div className="flex items-center gap-2.5">
                <span className="h-2 w-2 rounded-full bg-green-500"></span>
                <span className="text-slate-500">
                  Biaya Tenaga Kerja Langsung
                </span>
              </div>
              <span className="font-bold text-slate-800">
                {formatIDR(summary.total_btkl)}
              </span>
            </div>

            <div className="flex items-center justify-between text-[13px]">
              <div className="flex items-center gap-2.5">
                <span className="h-2 w-2 rounded-full bg-purple-500"></span>
                <span className="text-slate-500">Biaya Kemasan</span>
              </div>
              <span className="font-bold text-slate-800">
                {formatIDR(summary.total_packaging)}
              </span>
            </div>

            <div className="flex items-center justify-between text-[13px]">
              <div className="flex items-center gap-2.5">
                <span className="h-2 w-2 rounded-full bg-yellow-500"></span>
                <span className="text-slate-500">BOP Variabel</span>
              </div>
              <span className="font-bold text-slate-800">
                {formatIDR(summary.total_bop_var)}
              </span>
            </div>

            {calculation_method === "full" && (
              <div className="flex items-center justify-between text-[13px]">
                <div className="flex items-center gap-2.5">
                  <span className="h-2 w-2 rounded-full bg-red-500"></span>
                  <span className="text-slate-500">BOP Tetap</span>
                </div>
                <span className="font-bold text-slate-800">
                  {formatIDR(summary.total_bop_fix)}
                </span>
              </div>
            )}

            <div className="my-1 border-t border-dashed border-slate-200"></div>

            <div className="flex items-center justify-between text-[13px]">
              <span className="font-bold text-slate-800">
                Total HPP Keseluruhan
              </span>
              <span className="font-bold text-slate-800">
                {formatIDR(summary.total_hpp)}
              </span>
            </div>

            <div className="flex items-center justify-between text-[13px]">
              <span className="text-slate-500">HPP per {unitLabel}</span>
              <span className="font-bold text-slate-800">
                {formatIDR(summary.hpp_per_unit)}
              </span>
            </div>

            <div className="flex items-center justify-between text-[13px]">
              <span className="text-slate-500">Jumlah Produksi</span>
              <span className="font-bold text-slate-800">
                {qtyNumber} {unitLabel}
              </span>
            </div>
          </div>
        </div>

        <div className="flex h-full flex-col justify-between rounded-2xl bg-[#2563EB] p-5 text-white shadow-md xl:p-6">
          <div>
            <div className="mb-5 flex items-center gap-2">
              <Star className="h-5 w-5 text-yellow-400" fill="currentColor" />
              <h3 className="text-[15px] font-bold text-white">
                Rekomendasi Harga Jual
              </h3>
            </div>

            <div className="flex flex-col gap-3.5">
              <div className="flex items-center justify-between text-[13px]">
                <span className="text-blue-100">HPP Per {unitLabel}</span>
                <span className="font-bold text-white">
                  {formatIDR(summary.hpp_per_unit)}
                </span>
              </div>

              <div className="flex items-center justify-between text-[13px]">
                <span className="text-blue-100">
                  Target Margin ({margin_percentage}%)
                </span>
                <span className="font-bold text-white">
                  +{formatIDR(summary.profit_margin_value)}
                </span>
              </div>

              <div className="flex items-center justify-between text-[13px]">
                <span className="text-blue-100">
                  Keuntungan Per {unitLabel}
                </span>
                <span className="font-bold text-white">
                  {formatIDR(summary.profit_margin_value)}
                </span>
              </div>

              <div className="my-1 border-t border-dashed border-blue-400/50"></div>
              <div className="flex items-center justify-between">
                <span className="text-[14px] font-bold text-yellow-400">
                  Harga Jual Rekomendasi
                </span>
                <span className="text-[18px] font-bold text-yellow-400">
                  {formatIDR(summary.recommended_price)}
                </span>
              </div>
            </div>
          </div>

          <div className="mt-6 flex items-start justify-between rounded-xl bg-white/10 p-5">
            <div className="flex flex-col">
              <span className="mb-1 text-[11px] font-bold uppercase tracking-wider text-blue-200">
                Total Keuntungan
              </span>
              <span className="text-[24px] font-black leading-none tracking-tight text-white xl:text-[28px]">
                {formatIDR(summary.total_profit)}
              </span>
              <span className="mt-1.5 text-[10px] text-blue-200/80">
                ({qtyNumber} {unitLabel} ×{" "}
                {formatIDR(summary.profit_margin_value)})
              </span>
            </div>

            <div className="mx-3 h-14 w-px bg-blue-400/30 xl:mx-4"></div>

            <div className="flex flex-col text-right">
              <span className="mb-1 text-[11px] font-bold uppercase tracking-wider text-blue-200">
                Total Pendapatan
              </span>
              <span className="text-[24px] font-black leading-none tracking-tight text-white xl:text-[28px]">
                {formatIDR(summary.total_revenue)}
              </span>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
